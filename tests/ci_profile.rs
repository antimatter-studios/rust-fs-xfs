...
fn gating_runs_with_overflow_checks(workflow: &str) -> Vec<String> {
    gating_steps(workflow)
        .iter()
        .flat_map(|run| runs_with_overflow_checks(run))
        .collect()
}
...
fn gating_runs_that_prove_the_build_traps(workflow: &str) -> Vec<String> {
    gating_steps(workflow)
        .iter()
        .flat_map(|run| debug_runs_that_prove_the_build_traps(run))
        .collect()
}

/// The `run:` bodies of every step whose result gates a pull request.
fn gating_steps(workflow: &str) -> Vec<String> {
    ...
    wf.jobs
        .iter()
        .filter(|j| job_gates(j))
        .flat_map(|j| j.steps.iter().filter(|s| step_gates(s)))
        .map(|s| s.run.clone())
        .collect()
}
...
";
    ...
    ///
    /// This is why the check matches whole names rather than a prefix,
    /// and it is the same reason `pull_request_target` -- which also
    /// starts the same way -- is refused rather than assumed
    /// equivalent.
    #[test]

#!/usr/bin/env bash
#
# vm-build-create-fixtures.sh — run build-create-fixtures.sh inside the
...
guest_path="$("$REPO/scripts/vm.sh" put "$REPO/scripts/build-create-fixtures.sh")"
"$REPO/scripts/vm.sh" run "XFS_FIXTURE_DIR=/share bash '$guest_path'"
rm -f "$REPO/.vm-share/build-create-fixtures.sh"

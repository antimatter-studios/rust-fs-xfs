...
#
# THE ORACLE TOOLS ARE NOT HERE, AND THAT IS THE POINT. xfsprogs —
# mkfs.xfs, xfs_db, xfs_repair, xfs_logprint, xfs_io — runs inside the
# fs-linux-test-harness VM, provisioned by scripts/vm-setup.sh, and
# nowhere else. A workstation therefore installs none of it: no
# distribution build whose version differs from the next machine's, no
# locally compiled xfsprogs under ~/.local, and no chance of an oracle
# answering differently depending on who asked. tests/common/mod.rs is
# the only way a test reaches one, and tests/test_contract.rs fails the
# suite if anything runs one on the host.
#
# NOR IS sudo. This repository used to decide per host whether to run the
# kernel oracles natively with `sudo -n` or in the VM, which is how a
# CI runner and a developer's machine came to be judged by two different
# kernels. There is one kernel now and it is the guest's.
#

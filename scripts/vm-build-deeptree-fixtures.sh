#!/usr/bin/env bash
#
# vm-build-deeptree-fixtures.sh — run build-deeptree-fixtures.sh inside
# the oracle VM, for a host that has no xfsprogs and cannot loop-mount.
#
...
#
#   ./scripts/vm-build-deeptree-fixtures.sh
set -euo pipefail
...
guest_path="$("$REPO/scripts/vm.sh" put "$REPO/scripts/build-deeptree-fixtures.sh")"
"$REPO/scripts/vm.sh" run "XFS_FIXTURE_DIR=/share bash '$guest_path'"
rm -f "$REPO/.vm-share/build-deeptree-fixtures.sh"

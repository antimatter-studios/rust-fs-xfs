...
fn restamp_crc(buf: &mut [u8], crc_off: usize) {
...
/// correctly logged.
fn changed_chunks(blkno: u64, before: &[u8], after: Vec<u8>, buf_type: u16) -> BufferItem {
...
/// its root.
fn leaf_records(buf: &[u8], numrecs: u16) -> Vec<FreeExtent> {
...
fn rebuild_leaf(original: &[u8], records: &[FreeExtent]) -> Vec<u8> {
